# TASK_354 — RDD: Selector-mode legacy dual-alias case-normalization coverage

SPEC_EXPECTED: CODE

## Intent
Add bounded deterministic coverage for legacy dual-alias case-normalization handling so case-only formatting differences in allowed values are classified correctly while existing invalid/unsupported/whitespace-normalized paths remain regression-safe.
...
- Required negative cases:
  ...
  - canonical absent + both legacy aliases with equal allowed values that differ only by case fails closed with existing conflict marker.
  - canonical absent + both legacy aliases with different allowed values after case-normalization fails closed with existing mismatch marker.
...
- `docs/dev/evidence/RDD_PHASE18_SELECTOR_MODE_LEGACY_ALIAS_CASE_NORMALIZATION__v1/TASK_354/**`
...
- `docs/dev/evidence/RDD_PHASE18_SELECTOR_MODE_LEGACY_ALIAS_CASE_NORMALIZATION__v1/TASK_354/TESTS.txt`
- `docs/dev/evidence/RDD_PHASE18_SELECTOR_MODE_LEGACY_ALIAS_CASE_NORMALIZATION__v1/TASK_354/DIFF_NAME_ONLY.txt`
- `docs/dev/evidence/RDD_PHASE18_SELECTOR_MODE_LEGACY_ALIAS_CASE_NORMALIZATION__v1/TASK_354/DIFF_STAT.txt`
- `docs/dev/evidence/RDD_PHASE18_SELECTOR_MODE_LEGACY_ALIAS_CASE_NORMALIZATION__v1/TASK_354/HOTFILE_SCAN.txt`
...
## Constraints
- Coverage stays within bounded selector-mode legacy dual-alias case-normalization seam for the current v1 case class.
