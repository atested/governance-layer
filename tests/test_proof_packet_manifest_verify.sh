...
assert_contains "verify machine contract run1" "$OUT1" "PROOF_PACKET_VERIFY ok=yes reason=OK "
assert_contains "verify summary version run1" "$OUT1" " summary_report_version=proof_packet_verify_summary_v2 "
assert_contains "verify packet id run1" "$OUT1" " packet_id=ppb_"
assert_contains "verify summary sha run1" "$OUT1" " summary_sha256=sha256:"
