...
The proxy does not see the agent's internal state, local file operations that bypass the model, or interactions between the agent and other local processes. Governance covers the model-to-agent communication channel. Activity outside that channel is ungoverned unless the operator adds observation hooks (a separate mechanism that records ungoverned operations for transparency metrics but doesn't evaluate policy).
