...
- Top: /Users/gregkeeter/codex-workspaces/governance-layer
- Branch: main
...
- codex-batch.sh — #!/usr/bin/env bash
- codex-unattended.sh — #!/usr/bin/env bash
- codex-unattended.sh — #!/usr/bin/env bash
...
- merge-queue.sh — #!/usr/bin/env bash
- project-status.sh — #!/usr/bin/env bash
- project-status.sh — #!/usr/bin/env bash
- qt-runner.sh — #!/usr/bin/env bash
- qt-runner.sh — #!/usr/bin/env bash
...
system/scripts/codex-batch.sh:107:# Build batch instructions (READ-ONLY: do not claim tasks here)
system/scripts/codex-batch.sh:123:  # Find task file (no claim - read-only mode)
system/scripts/codex-batch.sh:137:   - FORBIDDEN: docs/dev/ASSIGNMENTS.md (Cecil updates this at merge time)
system/scripts/codex-batch.sh:150:Status: Ready for Codex (read-only list, no claims made)
...
system/scripts/project-status.sh:55:MERGE_QUEUE_FILES_RAW="$(find docs/dev/merge-queue -maxdepth 1 -type f -name '*.md' 2>/dev/null | sort || true)"
system/scripts/qt-runner.sh:187:    if p == "docs/dev/ASSIGNMENTS.md":
...
TASK_071__test_task_scaffold_with_executor_field.md
TASK_075__test_rc_fs_executable_disallowed.md
TASK_080__test_rc_fs_executable_disallowed.md
TASK_081__test_rc_fs_not_a_directory.md
TASK_092__test_rc_fs_executable_disallowed.md
...
docs/dev/evidence/TASK_071
docs/dev/evidence/TASK_072
docs/dev/evidence/TASK_073
docs/dev/evidence/TASK_074
docs/dev/evidence/TASK_075
docs/dev/evidence/TASK_076
docs/dev/evidence/TASK_077
docs/dev/evidence/TASK_078
docs/dev/evidence/TASK_090
docs/dev/evidence/TASK_091
docs/dev/evidence/TASK_092
