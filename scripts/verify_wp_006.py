#!/usr/bin/env python3
"""Run and report the immutable WP-006 validation manifest scenarios."""
...
ELEMENT_VALIDATORS = {
    "REQ-ATESTED-024": "LocalFirstContinuityValidator",
    "REQ-ATESTED-027": "ScopedEvidencePackageValidator",
}
...
def main() -> int:
    ...
    if unknown:
        print(f"unsupported WP-006 elements: {','.join(unknown)}", file=sys.stderr)
        return 2

    repo = Path(__file__).resolve().parents[1]
    project_python = repo / ".venv" / "bin" / "python"
    completed = subprocess.run(
        [str(project_python) if project_python.exists() else sys.executable, str(repo / "tests" / "wp_006_validator.py")],
    ...
    for element in requested:
        print(json.dumps({
            ...
            "evidence_refs": ["tests/wp_006_validator.py"],
