...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mcp-ingest-tool-event-rows.XXXXXX")"
...
open(out_path, 'w', encoding='utf-8').write(json.dumps(rows, sort_keys=True, separators=(',', ':')) + '\n')
PY
...
echo "MCP_INGEST_TOOL_EVENT_REPORTING_ROWS=PASS"
