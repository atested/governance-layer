#!/usr/bin/env python3
"""Run and report the immutable WP-005 validation manifest scenarios."""
...
ELEMENT_VALIDATORS = {
    "REQ-ATESTED-015": "OperatorDecisionReviewValidator",
    "REQ-ATESTED-016": "DashboardWindowModelValidator",
    "REQ-ATESTED-025": "DashboardInvestigationValidator",
    "REQ-ATESTED-026": "SummaryEvidenceTraceValidator",
}
...
def main() -> int:
    ...
    if unknown:
        print(f"unsupported WP-005 elements: {','.join(unknown)}", file=sys.stderr)
        return 2

    repo = Path(__file__).resolve().parents[1]
    project_python = repo / ".venv" / "bin" / "python"
    completed = subprocess.run(
        [str(project_python) if project_python.exists() else sys.executable, str(repo / "tests" / "wp_005_validator.py")],
    ...
    for element in requested:
        print(json.dumps({
            ...
            "evidence_refs": ["tests/wp_005_validator.py"],
