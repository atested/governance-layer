...
env ROOT="$ROOT" ISSUER_URL="$ISSUER_URL" "$VENV_PY" - <<'PY'
import os
import sys
...
token_response = client.post("/token", data={"grant_type": "authorization_code"})
assert token_response.status_code == 404, (token_response.status_code, token_response.text)

print("PASS: same-origin authorization-server metadata surfaces disabled")
print("PASS: same-origin authorize/token public routes disabled")
PY
