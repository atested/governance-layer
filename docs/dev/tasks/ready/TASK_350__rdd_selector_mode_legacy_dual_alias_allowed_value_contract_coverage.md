# TASK_350 — RDD: Selector-mode legacy dual-alias allowed-value contract coverage

SPEC_EXPECTED: CODE

## Intent
Add bounded deterministic coverage for legacy dual-alias allowed-value handling so unsupported dual legacy selector-mode values fail closed with stable markers while existing valid/invalid paths remain regression-safe.
...
- Required negative cases:
  ...
  - canonical absent + both legacy aliases with equal allowed values fails closed with existing conflict marker.
  - canonical absent + both legacy aliases with different allowed values fails closed with existing mismatch marker.
  ...
  - canonical absent + both legacy aliases where one or both values are unsupported non-empty strings fails closed with stable value-unsupported marker.
...
- `docs/dev/evidence/RDD_PHASE16_SELECTOR_MODE_LEGACY_ALIAS_ALLOWED_VALUE_CONTRACT__v1/TASK_350/**`
...
- `docs/dev/evidence/RDD_PHASE16_SELECTOR_MODE_LEGACY_ALIAS_ALLOWED_VALUE_CONTRACT__v1/TASK_350/TESTS.txt`
- `docs/dev/evidence/RDD_PHASE16_SELECTOR_MODE_LEGACY_ALIAS_ALLOWED_VALUE_CONTRACT__v1/TASK_350/DIFF_NAME_ONLY.txt`
- `docs/dev/evidence/RDD_PHASE16_SELECTOR_MODE_LEGACY_ALIAS_ALLOWED_VALUE_CONTRACT__v1/TASK_350/DIFF_STAT.txt`
- `docs/dev/evidence/RDD_PHASE16_SELECTOR_MODE_LEGACY_ALIAS_ALLOWED_VALUE_CONTRACT__v1/TASK_350/HOTFILE_SCAN.txt`
...
## Constraints
- Coverage stays within bounded selector-mode legacy dual-alias allowed-value seam for the current v1 case class.
