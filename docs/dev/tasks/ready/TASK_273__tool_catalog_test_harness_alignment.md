# TASK_273 — Tool-catalog test harness alignment

SPEC_EXPECTED: CODE

## Intent
Introduce shared test fixture/helpers or consistent test conventions for the merged tool-catalog suite so current-main tests stop duplicating avoidable setup logic. Improve continuation depth on the same surface without changing subsystem scope.

## Acceptance criteria
- Shared fixture/helper patterns reduce duplicated setup across touched tool-catalog tests.
- Fixture generation remains deterministic and self-contained.
- Tests do not depend on pre-existing `out/` artifacts unless created by the test.
- Scope remains bounded to existing tool-catalog test and directly-adjacent helper surfaces.

## Files allowed to touch
- system/tests/**
- scripts/attest/**
- mcp/tool_catalog_store.py
- docs/dev/evidence/TASK_273/**

## Files forbidden to touch
- capabilities/capability-registry.json
- mcp/server.py
- scripts/dev_phase2_regression.sh
- scripts/dev_generate_verification_catalog.py
- system/planning/verification_catalog.v1.json
- docs/dev/ASSIGNMENTS.md
- system/scripts/release-gate.sh
- system/scripts/validate-proof-bundle.sh
- system/scripts/codex-unattended.sh
- Everything else.

## Required evidence artifacts
- docs/dev/evidence/TASK_273/TESTS.txt
- docs/dev/evidence/TASK_273/DIFF_NAME_ONLY.txt
- docs/dev/evidence/TASK_273/DIFF_STAT.txt
- docs/dev/evidence/TASK_273/HOTFILE_SCAN.txt

## Determinism expectations
- Deterministic fixture behavior on touched tool-catalog tests.
- run1/run2 normalized output SHA256 must match where applicable.

## STOP rules
- STOP if implementation requires server wiring beyond `mcp/tool_catalog_store.py`, reporting enrollment, merge work, or doctrine changes.
- STOP if implementation requires hot-file edits.
- STOP if fixture changes broaden into unrelated subsystem surfaces.

## Constraints
- No merge work.
- No reporting enrollment.
- No doctrine changes.
- No MCP/server integration beyond `mcp/tool_catalog_store.py`.
