...
async def main():
    ...
    async with stdio_client(params) as (r3, w3):
        async with ClientSession(r3, w3) as session3:
            ...
            assert allow_obj["policy_decision"] == "ALLOW"
    ...
    after_n = chain_len()
    # Chain grows by: 1 fs_write ALLOW + 2 fs_list + 2 fs_read + 2 fs_mkdir + 2 fs_move = +9
    assert after_n == before_allow_n + 9, f"Expected chain to grow by 9 (1 fs_write + 2 fs_list + 2 fs_read + 2 fs_mkdir + 2 fs_move), before={before_allow_n}, after={after_n}"

    print("PASS: MCP smoke (DENY + ALLOW + tamper + fs_list + fs_read + fs_mkdir + fs_move tests passed, fail-closed verified)")
