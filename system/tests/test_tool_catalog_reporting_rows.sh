...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/tool-catalog-reporting.XXXXXX")"
...
open(rows_out, 'w', encoding='utf-8').write(json.dumps(rows, sort_keys=True, separators=(',', ':')) + '\n')
PY
...
echo "TOOL_CATALOG_REPORTING_ROWS=PASS"
